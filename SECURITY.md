...
**The sandbox key shares the in-flight ceiling with the live key.** `PROXLANE_SANDBOX_KEY` never reaches a provider, a provider key, or `/health/*`, and every sandbox response and log line says it is one. What it can do is hold `PROXLANE_MAX_INFLIGHT` slots at near-zero cost per request, since nothing waits on I/O, and shed live callers to `GATEWAY_BUSY`. On a self-hosted instance the holder of that key is the operator, which is only true while the key is private: a published sandbox key on a reachable server hands that capability to anyone. So the gateway warns at boot, and `proxlane doctor` flags it, when the sandbox key is a placeholder or a published value, and the try page generates its sandbox key rather than printing one. It warns rather than refuses because the sandbox cannot spend and a published key on localhost is harmless. If this key class is ever issued to strangers it needs its own ceiling first; that lands with the hosted tier, alongside the rate limiting above.
