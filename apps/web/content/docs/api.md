...
| `GET` | `/health` | Liveness and the running version. Needs no key |
...
| `X-Chain` | when a provider was tried | Every attempt as `provider:outcome`, in order |
...
| `X-Cost-Source` | when anything was charged | `reported` if the providers gave us the figure, `estimated` if we worked it out, `mixed` |
...
**`X-Chain` is the one that says who failed.** `X-Provider-Used` names the winner, so a
request that failed over and then succeeded returns a clean 200 that hides the provider which
cost you 22 seconds:
