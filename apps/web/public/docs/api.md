...
| `X-Chain` | always | Every attempt as `provider:outcome`, in order |
...
It is always present, and a single-attempt request has a one-element chain. That matters: an
absent header cannot say "nothing failed".
