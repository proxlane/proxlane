	...
	const spec = {
									...
									schema: {
										...
										properties: {
											...
											usable: {
												...
												minimum: 0,
												description:
													'How many of the configured providers are not in an account cooldown right now — the number a request could actually be sent to. `providers: 4, usable: 0` is a gateway whose credentials or wallets, not the targets, are stopping every request. Still a count and never a name; the names are behind the key on /health/cooldowns. Omitted when cooldowns are off or the state store is unreachable, because liveness must not depend on it.',
											},
										},
									},
								},
				...
				get: {
					...
					responses: {
						'200': {
							...
							content: {
								'application/json': {
									schema: { $ref: '#/components/schemas/ProviderHealthBody' },
								},
							},
						},
						'401': {
				...
				get: {
					...
					responses: {
						'200': {
							...
							content: {
								'application/json': {
									schema: { $ref: '#/components/schemas/CooldownsBody' },
								},
							},
						},
						'401': {
		...
		components: {
			...
			schemas: {
				...
				// THE TWO /health BODIES WERE UNTYPED, and they are the endpoints an operator's
				// tooling reads on a schedule. Both had a `200` with a description and no
				// content at all, so a generated client got `unknown` back from the only two
				// calls it would make repeatedly. Shapes taken from a running 0.10.0 container,
				// not from the handler's types, because the handler reshapes the cooldown key
				// into something neither the store nor the taxonomy declares.
				ProviderHealthBody: {
					type: 'object',
					required: ['providers'],
					properties: {
						providers: {
							...
							items: {
								type: 'object',
								required: ['id', 'state', 'baselineFailureRate', 'statistic', 'samplesInState'],
								properties: {
									id: { type: 'string', description: 'Adapter id, e.g. `scraperapi`.' },
									state: {
										type: 'string',
										enum: ['healthy', 'degraded', 'demoted'],
										description:
											'`degraded` moves the provider down the chain; `demoted` removes it unless it is the last capable one.',
									},
									baselineFailureRate: {
										type: ['number', 'null'],
										description:
											'The frozen p0 the statistic is measured against. Null until enough samples exist to bootstrap it.',
									},
									statistic: {
										type: 'number',
										description:
											'The accumulated CUSUM value. Crosses a threshold to change state.',
									},
									samplesInState: { type: 'integer' },
								},
							},
						},
					},
				...
				CooldownEntry: {
					type: 'object',
					required: ['scope', 'provider', 'expiresInMs', 'consecutive', 'probeTaken'],
					properties: {
						scope: {
							type: 'string',
							enum: ['domain', 'account'],
							description:
								"`domain` is a block fact and is shared; `account` is one org's key and is not.",
						},
						provider: { type: 'string' },
						domain: { type: 'string', description: 'Present when scope is `domain`.' },
						premium: {
							type: 'string',
							enum: ['none', 'residential', 'stealth'],
							description:
								'Present when scope is `domain`. A block at one tier does not imply the others.',
						},
						org: { type: 'string', description: 'Present when scope is `account`.' },
						expiresInMs: { type: 'integer', minimum: 0 },
						consecutive: {
							type: 'integer',
							description:
								'What makes the backoff exponential, and why an expired entry is still listed.',
						},
						probeTaken: {
							type: 'boolean',
				...
				CooldownsBody: {
					type: 'object',
					required: ['cooling', 'expired'],
					properties: {
						cooling: {
							type: 'array',
							items: { $ref: '#/components/schemas/CooldownEntry' },
						},
						expired: {
							type: 'array',
							items: { $ref: '#/components/schemas/CooldownEntry' },
							description:
								'Expired but still recorded: `consecutive` here is the reason the next cooldown on that key is longer.',
						},
					},
				},
				ErrorBody: {
